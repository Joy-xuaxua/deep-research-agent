    ...
    await runResearchStream(
      ...
      (event: ResearchStreamEvent) => {
        ...
        if (event.type === "archived") {
          const archiveDir = typeof event.archive_dir === "string" ? event.archive_dir : "";
          const taskCount = typeof event.task_count === "number" ? event.task_count : 0;
          if (archiveDir) {
            progressLogs.value.push(`笔记已归档至: ${archiveDir} (${taskCount} 个任务笔记)`);
          }
          return;
        }

        if (event.type === "archive_failed") {
          const errorMessage = typeof event.error === "string" ? event.error : "未知错误";
          progressLogs.value.push(`笔记归档失败: ${errorMessage}`);
          return;
        }
